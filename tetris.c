 ...
 *   	PLAYER *player_create()
 *   	
 *   	Crea un nuevo jugador de TETRIS. Esta funci�n devuelve un puntero 
 ...
 *
 * 	Inicializa la estructura para el JUGADOR. Esta funci�n 
...
void PLAYER_init(PLAYER *player)
{
	...
	/* Por default, el JUGADOR no est� rankeado
	 * NOT_RANKED es -1. Al darle el valor -2 es seguro que
	 * siempre va a escribirse la posici�n del JUGADOR */
...
void PLAYER_draw(PLAYER *player)
{
	...
	/* Chequear si hubo un ascenso en la posici�n del highscore
 ...
 *
 *	Mueve la FICHA activa en la direcci�n indicada 
 *	como par�metro. Devuelve el estado del juego despu�s de
 *	la movida.
 */
int PLAYER_movepiece(PLAYER *player, int direction)
{
...
void PLAYER_check(PLAYER *player)
{
	...
	for (j=0; j<board_height; j++) {
		...
		if (k != 0) {
			...
			/* Primero, se hace un efecto simpatic�n */
			memset(player->board+j*board_width, 8, board_width);
			player->draw_board = TRUE;
			PLAYER_draw(player);
			SDL_Delay(50);
			
			/* Ahora s� se quitan las l�neas */
			for (k=j; k>0; k--) 
			...
			/* Cada 10 l�neas se avanza un nivel. 
			 * El m�nimo drop_delay posible es 12. */ 
 ...
 *
 * 	Rota la FICHA hacia la izquierda. Esta funci�n no es para ubicar
 * 	la ficha en el tablero. Solamente calcula la m�scara de la ficha
 * 	rotada. 
 * 	FIXME: �direction?
 */
 ...
 *	como par�metro en la direcci�n especificada.
 *	Esta funci�n no ubica la ficha en el tablero. Solamente 
 *	calcula la nueva m�scara. 
 */
PIECE *PIECE_move(PIECE *piece, int direction)
{
 ...
 *
 * 	Extrae la ubicaci�n de los puntos de cada ladrillo en base
 * 	a la pieza y a su rotaci�n.
 */
void PIECE_getmask(PIECE *piece)
{
 ...
 *
 * 	Devuelven los extremos de la m�scara de la ficha.
 * 	Es necesario TENER la m�scara cargada en la estructura.
 */
int PIECE_getmaxx(PIECE *piece) 
{ 
 ...
 *
 * 	Dibuja el ladrillo number en la posici�n (x,y) de la pantalla. 
 * 	Los ladrillos est�n en el archivo "blocks.bmp". Los primeros 
 * 	7 ladrillos son de color (rojo, rosa, azul, celeste, verde,
 * 	amarillo y beige). Los 7 restantes son grises.
 * 	Esta funci�n necesita ser seguida de un UPDATE.
 *
 */
void BLOCK_draw(int number, int x, int y)
{
 ...
 * 	int BLOCK_random()
 *
 * 	Devuelve un n�mero entre 1 y 7
 */
int BLOCK_random()
{
